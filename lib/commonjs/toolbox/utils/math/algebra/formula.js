...
var Formula = (function () {
    ...
    Formula.fromString = function (formula) {
        if (!formulaRegex.test(formula)) {
            service_1.ErrorService.throw("Invalid formula \"" + formula + "\", does not match regex");
        }
    ...
    Formula.reduceFirstNegation_ = function (pieces) {
        if (pieces.length > 1 && pieces[0] === operation_1.Subtract) {
            ...
            if (!(firstValue instanceof variable_1.Variable)) {
                service_1.ErrorService.throw('Formulas should start with a Variable');
                return [];
            }
        ...
        for (var i = 1; i < length; i++) {
            ...
            if (current instanceof variable_1.Variable) {
                if (last instanceof variable_1.Variable) {
                    service_1.ErrorService.throw('Formula does not support implicit multiplication.');
                }
                    ...
                    service_1.ErrorService.throw('Formulas must have a value between operators.');
                }
                else {
                    insert(current);
                }
    ...
    Formula.reduceWithOperations_ = function (pieces, operations) {
        ...
        if (pieces.length === 1) {
            ...
            service_1.ErrorService.throw('Cannot reduce with operation on an even number of pieces');
        }
        ...
        for (var i = 2; i < pieces.length; i += 2) {
            ...
            if (!(lastVariable instanceof variable_1.Variable)) {
                service_1.ErrorService.throw('Found IOperation where a Variable was expected');
                return [];
            }
            if (!(current instanceof variable_1.Variable)) {
                service_1.ErrorService.throw('Found IOperation where a Variable was expected');
            ...
            if (operation instanceof variable_1.Variable) {
                service_1.ErrorService.throw('Found Variable where a IOperation was expected');
                return [];
            }
    ...
    Formula.reduceWithExecution_ = function (pieces) {
        ...
        if (pieces.length % 2 === 0) {
            service_1.ErrorService.throw('Cannot reduce with execution on an even number of pieces');
        }
