...
var Slide = (function (_super) {
    ...
    Slide.endInteraction_ = function (carousel) {
        ...
        if (Math.abs(gestureDistance) < GESTURE_MOVEMENT_THRESHOLD) {
            ...
            var slideDistance = function (slide) {
                return get_visible_distance_between_elements_1.getVisibleDistanceBetweenElements(slide, carousel.getContainer()).x;
            };
    ...
    Slide.initPosition_ = function (target, carousel) {
        render_loop_1.renderLoop.measure(function () {
            var translation = Slide.getTransitionTranslation_(target, carousel, Number.MAX_VALUE);
            Slide.transition_(target, carousel, translation);
        });
    };
    Slide.getTransitionTranslation_ = function (target, carousel, step) {
        var xDistance = -get_visible_distance_between_elements_1.getVisibleDistanceBetweenElements(target, carousel.getContainer()).x;
    ...
    Slide.transitionBeforeSlide_ = function (slideToTransition, activeSlide, previousSlides, translation) {
        var currentOffset = get_visible_distance_between_elements_1.getVisibleDistanceBetweenElements(slideToTransition, activeSlide);
