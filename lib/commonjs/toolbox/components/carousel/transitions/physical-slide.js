...
var PhysicalSlide = (function () {
    ...
    PhysicalSlide.getTranslationFromCenter_ = function (target, carousel) {
        var xDistance = get_visible_distance_between_element_centers_1.getVisibleDistanceBetweenElementCenters(target, carousel.getContainer())
            .x;
        return new vector_2d_1.Vector2d(xDistance, 0);
    };
    ...
    PhysicalSlide.prototype.initDraggableSlides_ = function (carousel) {
        ...
        carousel.getSlides()
            ...
            event_handler_1.eventHandler.addListener(draggable, drag_1.Drag, function (event) { return slide_1.Slide.handleDrag(event, carousel); });
    ...
    PhysicalSlide.prototype.transitionToTarget_ = function (carousel) {
        ...
        var predictedState = draggable.getPhysical2d().predictStateInXMilliseconds(remainingTime);
        var expectedEndDistance = distanceToCenter - predictedState.getDistanceTraveled().x;
    ...
    PhysicalSlide.prototype.adjustSplit_ = function (carousel) {
    ...
    PhysicalSlide.prototype.getAdjustedSlidePositions_ = function (carousel) {
        var activeSlide = carousel.getActiveSlide();
        var halves = split_evenly_on_item_1.splitEvenlyOnItem(carousel.getSlides(), activeSlide);
        var slidesBefore = halves[0];
        var slidesAfter = halves[1];
        this.getSlideAdjustments_(activeSlide, slidesBefore.reverse(), -1);
        this.getSlideAdjustments_(activeSlide, slidesAfter, 1);
    };
    ...
    PhysicalSlide.prototype.adjustSlides_ = function (activeSlide, slides, direction) {
        var _this = this;
        Array.from(this.getSlideAdjustments_(activeSlide, slides, direction).entries())
            ...
            _this.draggableBySlide_
                .get(slide).adjustNextFrame(new vector_2d_1.Vector2d(adjustment, 0));
        });
    };
