...
var QueryParameters = (function () {
    ...
    QueryParameters.getQueryParams = function (reload) {
        if (reload === void 0) { reload = {}; }
        var url = QueryParameters.getUrl_(reload);
        var queryParamString = url.split('?').slice(-1)[0];
    ...
    QueryParameters.setParameterByName = function (rawName, rawValue, reload) {
        ...
        var queryParams = QueryParameters.getQueryParams(reload);
        var name = QueryParameters.escapeParamString_(rawName);
    ...
    QueryParameters.updateUrl = function (queryParams, reload) {
        if (reload === void 0) { reload = false; }
        var paramsString = Object.keys(queryParams)
            .map(function (key) { return [key, queryParams[key]].join('='); })
            .join('&');
        if (!reload) {
            window.history.pushState({ 'path': window.location.origin + window.location.pathname }, '', "?" + paramsString);
        }
        else {
