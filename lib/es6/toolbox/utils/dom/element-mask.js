...
var ElementMask = (function () {
    function ElementMask(fixedElement, maskElement, buffer) {
        ...
        this.maskVisibleDimensions_ =
            VisibleDimensions.getForElement(maskElement);
        this.maskPosition_ =
            VisibleDistance.getForElement(maskElement);
        this.stopped_ = false;
        this.buffer_ = buffer;
        this.init_();
    }
    ...
    ElementMask.prototype.renderFixed_ = function () {
        ...
        if (this.maskVisibleDimensions_.getDimensions().getLength() != 0) {
            bufferedDimensions = (_b = this.maskVisibleDimensions_.getDimensions()
                .add(new Dimensions2d(this.buffer_, this.buffer_))).clamp.apply(_b, this.maskDimensions_.getDimensions().asRanges());
        }
        renderLoop.mutate(function () {
