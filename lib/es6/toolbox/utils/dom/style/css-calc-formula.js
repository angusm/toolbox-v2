...
var CssCalcFormula = (function () {
    ...
    CssCalcFormula.fromVariables = function () {
        ...
        return new (CssCalcFormula.bind.apply(CssCalcFormula, [void 0].concat(ALLOWED_UNITS.map(function (allowedUnit) { return mappedValues.get(allowedUnit); }))))();
    };
    ...
    CssCalcFormula.fromStyleString = function (styleString) {
        var formula = styleString.slice(0, 4) === 'calc' ?
            ...
            styleString;
        var formulaPieces = Formula.fromString(formula).reduce().getPieces();
        var variables = formulaPieces
            .filter(function (value) { return contains(ALLOWED_UNITS, value.symbol); })
            .filter(function (value) { return value instanceof Variable; });
        var orderedVariables = ALLOWED_UNITS.map(function (allowedUnit) {
    ...
    CssCalcFormula.prototype.toStyleString = function () {
        var values = zip(this.values_, ALLOWED_UNITS)
            .filter(function (_a) {
