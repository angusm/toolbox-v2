...
var FrameSequenceBg = (function () {
    function FrameSequenceBg(frames, container, _a) {
        ...
        this.reloadFrameMap_ =
            MultiValueDynamicDefaultMap.usingFunction(function (_a) {
        ...
        return loadImage(frameUrl)
            ...
            _this.desiredFrameForTarget_.forEach(function (desiredFrame, target) {
                if (desiredFrame === frameToLoad) {
                    ...
                    var currentBack = _this.loadedBackFrameForTarget_.get(target);
                    var currentFront = _this.loadedFrontFrameForTarget_.get(target);
                    if ((currentBack < frameToLoad && frameToLoad < desiredFrame) ||
                        (desiredFrame < frameToLoad && frameToLoad < currentFront)) {
                        _this.updateWithMissingFrame_(target, _this.lastDistanceAsPercentForTarget_.get(target), desiredFrame);
                    }
                }
            });
    ...
    FrameSequenceBg.prototype.clearCachedValuesForMissingFrame_ = function (target) {
    ...
    FrameSequenceBg.prototype.updateBackFrameWithMissingFrame_ = function (target, frameToDisplay) {
        var _this = this;
        this.loadedBackFrameForTarget_.set(target, frameToDisplay);
        var imageUrl = "url(" + this.imageUrlsInOrder_[frameToDisplay] + ")";
        renderLoop.anyMutate(function () {
            setStylesFromMap(_this.backFrame_, new Map([['background-image', imageUrl], ['opacity', '1']]));
        });
    };
    FrameSequenceBg.prototype.updateFrontFrameWithMissingFrame_ = function (target, frameToDisplay, opacity) {
        var _this = this;
        this.loadedFrontFrameForTarget_.set(target, frameToDisplay);
        var imageUrl = "url(" + this.imageUrlsInOrder_[frameToDisplay] + ")";
        renderLoop.anyMutate(function () {
            setStylesFromMap(_this.frontFrame_, new Map([['background-image', imageUrl], ['opacity', "" + opacity]]));
        });
    };
    FrameSequenceBg.prototype.mutateUsingStyleMaps_ = function (backStyle, frontStyle) {
        var _this = this;
        renderLoop.anyMutate(function () {
            setStylesFromMap(_this.backFrame_, backStyle);
            setStylesFromMap(_this.frontFrame_, frontStyle);
        });
    };
